...
async fn get_credit_applications(
    ...
    sqlx::query!(
        ...
        FROM
          ...
          LEFT JOIN (
            SELECT
              DISTINCT ON (transactions.args ->> 'account_id') transactions.args ->> 'account_id' AS project_id,
              transactions.timestamp AS timestamp
            FROM
              transactions
            WHERE
              transactions.method_name = 'apply_for_program'
            ORDER BY
        ...
        WHERE
          (
            projects.application ILIKE '%Submitted%'
            AND projects.application NOT ILIKE '%NotSubmitted%'
          )
